<?php 
	include 'dbconn.php';
	$id=$_GET['updateuser'];
	$sql = "SELECT * FROM test_confirmation WHERE CONFID = '$id'";
    ...
    $selectedPriest = $row['PRIEST'];

	$motherrow =  $row['MOTHER'];
    $fatherrow = $row['FATHER'];
    $babyrow = $row['CHILD'];
    $birthdaterow = $row['BIRTHDATE'];
    $confdaterow = $row['CONFDATE'];
    $priestrow = $row['PRIEST'];
    $ninongrow = $row['GODFATHER'];
    $ninangrow = $row['GODMOTHER'];
	...
	if (isset($_POST['submit'])) {
        ...
        //FILE UPLOAD
        //baptismal
        // $bapcertfileerror = $baptismalfile['error'];
        // $bfilepath = "SBWedding/";
		// $bfilename = $_FILES['baptismal']['name'];
		// $btmpname = $_FILES['baptismal']['tmp_name'];
		// $bfiletmp = addslashes(file_get_contents($btmpname));
		// $bfiletarget = $bfilepath . basename($_FILES["baptismal"]["name"]);
		// $buploadCheck = 1;
		// $bimageFileType = strtolower(pathinfo($bfiletarget,PATHINFO_EXTENSION));
        // $bfinal_file = "SBWedding/" .$bfilename;
        ...
        $checkquery = "SELECT * from test_confirmation WHERE CONFDATE = '$confdate'";
        ...
        if ($rs)
        {
            ...
            {
                $sql = "UPDATE test_confirmation SET CONFID = '$id',
                CHILD = '$baby',
                FATHER = '$father', 
                MOTHER = '$mother',
                BIRTHDATE = '$birthdate',
        ...
        <div class="d-flex flex-column" id="content-wrapper">
            <div id="content">
                ...
                <div class="container-fluid">
                    <h3 class="text-dark mb-4">Confirmation Service</h3>
                    <div class="row mb-3">
                        <div class="col-lg-4 col-xl-3">
                            <div class="card mb-3">
                                <div class="card-body text-center shadow"><img class="rounded-circle mb-3 mt-4" src="assets/img/dogs/san%20bartolome.jpg" width="160" height="160">
                                    <div class="mb-3"></div><strong>San Bartolome Confirmation</strong>
                                </div>
                            </div>
                            <div class="card"><img class="card-img-top w-100 d-block" src="img/confirmation%20logo.jpg" width="289" height="155">
                                <div class="card-body">
                                    <h4 class="card-title">What is Confirmation</h4>
                                ...
                                <div class="col">
                                    <div class="card shadow mb-3">
                                        ...
                                        <div class="card-body">
                                                ...
                                                <div class="row">
                                                    <div class="col">
                                                        <div class="mb-3"><label class="form-label" for="username"><strong>Child name</strong></label><input class="form-control" type="text" id="username-1" placeholder="Name of Child" value="<?php echo $babyrow ?>" pattern="[A-Za-z ]{1,}" name="child" required></div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="mb-3"><label class="form-label" for="email"><strong>Father's name</strong></label><input class="form-control" type="text" id="email-1" placeholder="Name of Father" value="<?php echo $fatherrow ?>" pattern="[A-Za-z ]{1,}"  name="father" required></div>
                                                    </div>
                                                </div>
                                                <div class="row">
                                                    <div class="col">
                                                        <div class="mb-3"><label class="form-label" for="first_name"><strong>Mother's name</strong></label><input class="form-control" type="text" id="first_name-1" placeholder="Name of Mother" value="<?php echo $motherrow ?>" pattern="[A-Za-z ]{1,}"  name="mother" required></div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="mb-3"><label class="form-label" for="last_name"><strong>Date of Birth</strong></label><input class="form-control" type="date" id="date" max="<?= date('Y-m-d'); ?>" value="<?php echo $birthdaterow ?>"  name="birthdate" required></div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="card-body">
                                                ...
                                                <div class="row">
                                                    <div class="col">
                                                        <div class="mb-3"><label class="form-label" for="last_name"><strong>Date of Confirmation</strong></label><input class="form-control" type="date" id="date" min="<?= date('Y-m-01'); ?>" max="<?= date('Y-m-t', strtotime('August')); ?>" value="<?php echo $confdaterow ?>"  name="confdate" required></div>
                                                    </div>
                                                    <div class="col">
                                                        <div class="mb-3">
                                                            <label class="form-label" for="username"><strong>Name of Priest</strong></label>
                                                            <select class="form-select" name="priest" id="priest">
                                                                <optgroup label="Choose your Priest">
                                                                    <?php
                                                                    while ($pastorRow = mysqli_fetch_array($resultquery)) {
                                                                        $pastorName = $pastorRow['PASTOR'];
                                                                        $selected = ($pastorName == $selectedPriest) ? 'selected' : ''; // Check if the pastor is selected

                                                                        echo "<option value='$pastorName' $selected>$pastorName</option>";
                                                                    }
                                                                    ?>
                                                                </optgroup>
                                                            </select>
                                        ...
                                        <div class="card-body">
                                            ...
                                            <div class="row">
                                                <div class="col">
                                                    <div class="mb-3"><label class="form-label" for="last_name"><strong>Name of Godfather</strong></label><input class="form-control" type="text" id="date" value="<?php echo $ninongrow ?>" pattern="[A-Za-z ]{1,}"  name="ninong" required></div>
                                                </div>
                                                <div class="col">
                                                    <div class="mb-3"><label class="form-label" for="username"><strong>Name of Godmother</strong></label><input class="form-control" type="text" id="birthcertbaby" value="<?php echo $ninangrow ?>" pattern="[A-Za-z ]{1,}"  name="ninang" required></div>
                                                </div>
                                                <div class="col">
                                                    <div class="mb-3"><label class="form-label" for="username"><strong>Contact Number</strong></label><input class="form-control" type="text" id="birthcertbaby" placeholder="Ex. 09123456789" pattern="[0-9]{11}" value="<?php echo $contactrow ?>" name="contact" required></div>
                                                </div>
                                            </div>
                                                <div class="mb-3"></div>
                                            <button class="btn btn-primary btn-sm" type="submit" name="submit" onclick="return confirm('Are all the information correct?')">Update</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
